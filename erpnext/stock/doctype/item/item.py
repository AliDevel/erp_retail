...
class Item(Document):
	...
	def after_insert(self):
		...
		if self.standard_rate:
			self.add_price('Standard Selling',self.standard_rate)
			#for default in self.item_defaults or [frappe._dict()]:
				#self.add_price(default.default_price_list)
		if self.standard_buying:
			self.add_price('Standard Buying',self.standard_buying)			
	...
	def add_price(self, price_list=None, rate = 0):
		"""Add a new price"""
		...
		if price_list:
			# Calculate foreing rate 
			foreign_rate=flt(flt(rate)/ get_exchange_rate('USD',erpnext.get_default_currency()))
